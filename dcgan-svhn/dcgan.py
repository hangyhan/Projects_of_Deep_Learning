...
class Dataset:
    def __init__(self, train, test, val_frac=0.5, shuffle=False, scale_func=None):
        ...
        self.test_x, self.valid_x = test['X'][:, :, :,
                                              :split_idx], test['X'][:, :, :, split_idx:]
...
def model_inputs(real_dim, z_dim):
    inputs_real = tf.placeholder(
        tf.float32, (None, *real_dim), name='input_real')
...
def generator(z, output_dim, reuse=False, alpha=0.2, training=True):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        logits = tf.layers.conv2d_transpose(
            inputs=x3, filters=output_dim, kernel_size=5, strides=2, padding='same')
...
def model_loss(input_real, input_z, output_dim, alpha=0.2):
    ...
    d_model_fake, d_logits_fake = discriminator(
        g_model, reuse=True, alpha=alpha)

    d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        logits=d_model_real, labels=tf.ones_like(d_model_real)))
    d_loss_fake = tf.reduce_mean(tf.nn.sigmoid(
        logits=d_model_fake, label=tf.zeros_like(d_model_fake)))
    g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        logits=d_model_fake, labels=tf.ones_like(d_model_fake)))
